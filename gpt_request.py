...
def collect_response_from_gpt(db_path_list, question_list, api_key, engine, knowledge_list=None, correct_sql_list=None):
    responses_dict = {}
    response_list = []
    ...
    for i, question in tqdm(enumerate(question_list)):
        ...
        while attempt < 3 and not is_normally_correct:
            ...
            if correct_sql_list[i] is not None:
...
def decouple_question_schema(datasets, db_root_path):
    ...
    db_path_list = []
    knowledge_list = []
    correct_sql_list = []  # Add a list to store the correct SQL queries
    ...
    for i, data in enumerate(datasets):
        ...
        if correct_sql is None:
            raise ValueError(f"Missing 'SQL' field for data at index {i}")
        correct_sql_list.append(correct_sql)
        #correct_sql_list.append(data['SQL'])  # Extract the correct SQL

    return question_list, db_path_list, knowledge_list, correct_sql_list
...
if __name__ == '__main__':
    ...
    question_list, db_path_list, knowledge_list, correct_sql_list = decouple_question_schema(datasets=eval_data, db_root_path=args.db_root_path)
    assert len(question_list) == len(db_path_list) == len(knowledge_list)

    responses, feedback = collect_response_from_gpt(db_path_list, question_list, args.api_key, args.engine, knowledge_list, correct_sql_list)
...
with open(feedback_file_path, 'w') as file:
    for question, result in feedback.items():
